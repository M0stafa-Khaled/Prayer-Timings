...
citiesList.addEventListener("change", (city) => {
  ...
  axios
    .get(
      `https://api.aladhan.com/v1/timingsByCity?city=${city.target.value}&country=eg&method=5`
    )
    .then(GetPrayerTimes)
...
function showError(error) {
  ...
  switch (error.code) {
    case error.PERMISSION_DENIED:
      errorShow.innerHTML =
        " رفض المستخدم طلب تحديد الموقع الجغرافى تلقائيا لعرض الوقت اسمح بتحديد الموقع او اختر المدينة";
      result.classList.add("show");
      break;
